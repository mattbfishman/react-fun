...
function PeopleDialog({ref, setValue}) {
    ...
    return (
        <dialog ref={ref}>
            <div>
                <div className="PeopleDialog__container" onClick={(e) => {handleButtonClick(e)}}>
                    {PhoneRange.map((num) => {
                        const displayNumber = num + 1;
                        const display = displayNumber === 1 ? `${displayNumber} person` : `${displayNumber} people`
                        return(
                            <button type="button" className='PeopleDialog__button' key={`people:${displayNumber}`} value={display}>{displayNumber}</button>
                        )
                    })}
                </div>
            </div>
